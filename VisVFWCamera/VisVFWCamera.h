...
public:
	...
	//------------------------------------------------------------------
	// Public synchronization methods (used internally)
...
private:
	...
	HANDLE m_hEventWindowCreatedOrDestroyed;
	HANDLE m_hThread;
	DWORD m_dwThreadID;
	HWND m_hwndPreview;
	...
	bool m_fCheckForDecompressorWhenFormatChanges;
	bool m_fOnlyRGBCanBeTopFirst;
	bool m_fOnlyRGBCanBeBottomFirst;
	bool m_fFirstFrameAfterFormatChange;
	...
	HIC LocateDecompressor(int cbDest, BITMAPINFOHEADER* pbmihSrc = 0) const;
	void CreateAndBeginThread(void);
	void QuitAndDestroyThread(void);
};
