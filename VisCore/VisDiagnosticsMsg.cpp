...
void CVisString::RemoveTrailingNewlineCharacters(bool fOnlyRemoveOne)
{
	// Helper function to remove trailing newline characters.
	if (!empty())
	{
		iterator itLim = end();
		iterator itCur = itLim;
		if (*(itCur - 1) == '\n')
			-- itCur;
		if (!fOnlyRemoveOne)
		{
			while ((itCur != begin()) && (*(itCur - 1) == '\n'))
				-- itCur;
		}
...
{
	if(VisPGlobals()->DiagnosticsEnabled())
	{
		CVisString strWarning;
		va_list argList;
		va_start(argList, szFormat);
		strWarning.Format(szFormat, argList);
		strWarning.RemoveTrailingNewlineCharacters();
...
void CVisDiagnosticMsg::Error(int iLevel, const char *szFormat, ...)
{
	if(VisPGlobals()->DiagnosticsEnabled())
	{
		...
		strError.RemoveTrailingNewlineCharacters();
...
void CVisDiagnosticMsg::Trace(int iLevel, const char *szFormat, ...)
{
	if(VisPGlobals()->DiagnosticsEnabled())
	{
		...
		strTrace.RemoveTrailingNewlineCharacters();
...
void CVisDiagnosticMsg::ProgressReport(int iLevel, int nCur, int nLim, const char *szFormat, ...)
{
	if(VisPGlobals()->DiagnosticsEnabled())
	{
		...
		strProgress.RemoveTrailingNewlineCharacters();
