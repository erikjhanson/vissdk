...
{
	CVisMemBlock memblock(cbSize, ibOffset, ulFileMappingID, ullFileMappingAppID);
	return memblock;
}
...
CVisRefCntMemBlock::CVisRefCntMemBlock(void *pbData, int cbData,
  ...
  : m_cRef(1),
	...
	m_pvUser(pvUser),
	m_ulFileMappingID(0),
	m_ullFileMappingAppID(VisPGlobals()->FileMappingAppID())
{
...
CVisRefCntMemBlock::CVisRefCntMemBlock(int cbData, int ibOffset, 
  ...
  : m_cRef(1),
	...
	m_ullFileMappingAppID(ullFileMappingAppID)
{
	char szBuf[32];
	FillFileMappingNameString(szBuf, FileMappingPrefixString(), 
							  ulFileMappingID, ullFileMappingAppID);
...
{
	return VisPGlobals()->UseNamedFileMappings();
}

void CVisRefCntMemBlock::SetUseNamedFileMappings(bool f)
{
	VisPGlobals()->SetUseNamedFileMappings( f );
}
...
{
	assert(cbData > 0);
	if((m_cbData + 8) <= VisPGlobals()->CbImageSizeLimit())
	{
		...
		if (m_evismemblock == evismemblockAllocFileMapping)
		{
			...
			long lFileMappingID = VisPGlobals()->NextFileMappingID();
