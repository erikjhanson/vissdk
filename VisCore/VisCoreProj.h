...
//---------------------------------------------------------------------
// System (Windows and C runtime) include files
...
// UNDONE:  Move these someplace else.

#define gvis_PI 3.1415926535897932384626433832795
#define gvis_TwoPI (gvis_PI * 2.0)
#define gvis_HalfPI (gvis_PI / 2.0)
#define gvis_QuarterPI (gvis_PI / 4.0)

// These will treat numbers slightly < -1.0 or > 1.0 as -1.0 or 1.0.
inline double VisACos(double dbl)
	{ return ((dbl > -1.0) ? ((dbl < 1.0) ? acos(dbl) : 0.0) : gvis_TwoPI); }

inline double VisASin(double dbl)
	{ return ((dbl > -1.0) ? ((dbl < 1.0) ? asin(dbl) : gvis_HalfPI) : - gvis_HalfPI); }

// This will treat numbers slightly less than zero as equal to zero.
inline double VisSqrt(double dbl)
	{ return ((dbl > 0.0) ? sqrt(dbl) : 0.0); }


//---------------------------------------------------------------------
// VisCore project files
