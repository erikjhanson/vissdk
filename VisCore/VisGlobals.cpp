...
#ifdef _DEBUG
	OutputDebugString(
" ** VisSDK Note:  If you get two small memory leaks reported above at exit (of about 33 and\n"
			);
	OutputDebugString(
"    40 bytes), they can be ignored.  (They're related to C++ streams used in global objects.)\n"
			);
#endif // _DEBUG
}
...
void CVisGlobals::InitFileHandlers(void)
{
	if(m_nInternalFileHandlers == 0)
	{
		...
		// Ijl, if available
		extern CVisFileHandler *Ijl_FileHandler_Init();
		AddInternalFileHandler(Ijl_FileHandler_Init());
...
void CVisGlobals::IplInit(void)
{
	...
	if (m_hModuleIpl != 0)
	{
		...
		m_pfnIplDeallocateHeader = (void *)
				GetProcAddress(m_hModuleIpl, "iplDeallocate");
		m_pfnIplCreateROI = (void *)
				GetProcAddress(m_hModuleIpl, "iplCreateROI");
		m_pfnIplCloneImage = (void *)
				GetProcAddress(m_hModuleIpl, "iplCloneImage");
...
void *CVisGlobals::PfnIplCreateROI(void)
{
	if (!m_fCheckedForIpl)
	{
		IplInit();
	}
	return m_pfnIplCreateROI;
}

void *CVisGlobals::PfnIplCloneImage(void)
{
	if (!m_fCheckedForIpl)
	{
		IplInit();
	}
	return m_pfnIplCloneImage;
}

void CVisGlobals::IplFree(void)
{
	if (m_hModuleIpl)
	{
		FreeLibrary(m_hModuleIpl);
...
{
	if(m_ullFileMappingAppID == 0)
	{
		((unsigned long *) &m_ullFileMappingAppID)[0] = GetCurrentProcessId();
		assert(m_ullFileMappingAppID);
		((unsigned long *) &m_ullFileMappingAppID)[1] = GetTickCount();
	}
	return m_ullFileMappingAppID;
}
...
VisPropListExportDeclspec CVisGlobals *VisPGlobals(void)
{
	return &gvis_globals;
}
