...
typedef _IplImage* (__stdcall *TVisPfnIplCloneImage) (_IplImage* image);

void VISAPI VisIplMemCallback(void *pb, void *pvUser)
{
	assert(pb != 0);
#ifdef VIS_IPL_IPL_H
	_IplImage* piplCloneImage = (_IplImage*) pvUser;
	assert(piplCloneImage->imageDataOrigin == pb);

	assert(VisPGlobals()->PfnIplDeallocateHeader() != 0);

	((TVisPfnIplDeallocateHeader)
			VisPGlobals()->PfnIplDeallocateHeader())(piplCloneImage, IPL_IMAGE_ALL);
#else // VIS_IPL_IPL_H
	assert(0);
#endif // VIS_IPL_IPL_H
}
...
{
	return VisPGlobals()->IsAlphaWritten();
}

VisCoreExport void CVisImageBase::SetIsAlphaWritten(bool f)
{
	VisPGlobals()->SetIsAlphaWritten(f);
}
...
VisCoreExport void CVisImageBase::Alias(_IplImage *piplimage)
{
...
#ifdef VIS_IPL_IPL_H
	assert(piplimage != 0);
	CVisImageBase *pimagebase = (CVisImageBase *) piplimage->imageId;
	if(pimagebase != 0)
	{
		Alias(*pimagebase);
	}
	...
	{
		TVisPfnIplCloneImage pfnIplCloneImage
				= (TVisPfnIplCloneImage) VisPGlobals()->PfnIplCloneImage();
		if (pfnIplCloneImage != 0)
		{
			_IplImage* piplCloneImage = pfnIplCloneImage(piplimage);
			m_memblock = 
					CVisMemBlock(piplCloneImage->imageDataOrigin,
								 piplCloneImage->imageSize,
								 VisIplMemCallback, piplCloneImage);

			CVisRect visROIRect;
			if(piplCloneImage->roi != 0)
			{
				_IplROI* pIplROI = piplCloneImage->roi;
				visROIRect = CVisRect(pIplROI->xOffset, pIplROI->yOffset, 
									pIplROI->xOffset + pIplROI->width,
									pIplROI->yOffset + pIplROI->height);
			}
			else
			{
				visROIRect = CVisRect(0, 0, piplCloneImage->width, piplCloneImage->height);
			}
			ComputeAliasShapes(piplCloneImage->depth / 8, visROIRect,
							   CVisRect(0, 0, piplCloneImage->width, piplCloneImage->height));
...
VisCoreExport const unsigned long *CVisImageBase::MpbulColorMap(int cbitPixel) const
{
	...
	if ((m_fUseColorMap) && (ColorMap().CbData() >= 1024))
		...
		mpbulColorMap = VisPGlobals()->ColorMapGray8bit();
	else if (m_fUseColorMap)
		mpbulColorMap = VisPGlobals()->ColorMapDefault4bit();
	else
		mpbulColorMap = VisPGlobals()->ColorMapGray4bit();
		...
		const CVisShape& refshape)
{
	...
	if((dwMem > VisPGlobals()->CbImageSizeLimit()) ||
...
//
////////////////////////////////////////////////////////////////////////////
VisCoreExport CVisImageBase::~CVisImageBase(void)
{
	Cleanup();
}
...
//
////////////////////////////////////////////////////////////////////////////
VisCoreExport void CVisImageBase::SetName(const char *name)
{
                                                   ...
                                                   bool fDisplayErrorMessage)// @parm if 1 display error message, if 0 don't.
{
    ...
    try
	{
        ...
        else
        {
            ...
            if (fd.jpeg_quality == 0)
			{
				if (fd.default_jpeg_quality > 0)
			...
			{
				CVisFileHandler* pPictureHandler = VisPGlobals()->PictureFileHandler();
			...
			else
			{
				...
				VisPGlobals()->ReleasePictureFileHandler();
			}
		...
		const CVisImage<CVisRGBA<BYTE> >& refimageRGBA)
{
	...
	if (refimageRGBA.IsValid())
	{
		...
		VisMap2(CVisFnOpRGBABrightness(), imageGray,
