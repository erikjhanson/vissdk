		...
		case WM_COMMAND:
		{
			...
			switch(LOWORD(wParam))
			{
				case IDC_WARNING_EDIT:
				{
					...
					{
						VisPGlobals()->SetWarningConsoleLevel(uValue);
					}
					break;
				}
				case IDC_ERROR_EDIT:
				{
					...
					if(fTranslated && (uValue <= 16))
					{
						VisPGlobals()->SetErrorConsoleLevel(uValue);
					}
					break;
				}
				case IDC_PROGRESS_EDIT:
				{
					...
					if(fTranslated && (uValue <= 16))
					{
						VisPGlobals()->SetProgressConsoleLevel(uValue);
					}
					break;
				}
				case IDC_TRACE_EDIT:
				{
					...
					if(fTranslated && (uValue <= 16))
					{
						VisPGlobals()->SetTraceConsoleLevel(uValue);
					}
					break;
				}
			}
			...
			switch(((NMHDR FAR*)lParam)->code)
			{
				case UDN_DELTAPOS:
				{
					...
					switch(lpnmud->hdr.idFrom)
					{
						...
						{
							VisPGlobals()->SetWarningConsoleLevel(lpnmud->iPos + lpnmud->iDelta);
							break;
						}
						case IDC_ERROR_SPIN:
						{
							VisPGlobals()->SetErrorConsoleLevel(lpnmud->iPos + lpnmud->iDelta);
							break;
						}
						case IDC_PROGRESS_SPIN:
						{
							VisPGlobals()->SetProgressConsoleLevel(lpnmud->iPos + lpnmud->iDelta);
							break;
						}
						case IDC_TRACE_SPIN:
						{
							VisPGlobals()->SetTraceConsoleLevel(lpnmud->iPos + lpnmud->iDelta);
							break;
						}
					}
					break;
...
void CVisConsoleWnd::CreateControls(HWND hConsoleWnd)
{
	...
	else
	{
		...
		SetDlgItemInt(hConsoleWnd, IDC_WARNING_EDIT, VisPGlobals()->WarningConsoleLevel(), FALSE);
		SetDlgItemInt(hConsoleWnd, IDC_ERROR_EDIT, VisPGlobals()->ErrorConsoleLevel(), FALSE);
		SetDlgItemInt(hConsoleWnd, IDC_PROGRESS_EDIT, VisPGlobals()->ProgressConsoleLevel(), FALSE);
		SetDlgItemInt(hConsoleWnd, IDC_TRACE_EDIT, VisPGlobals()->TraceConsoleLevel(), FALSE);
	}
