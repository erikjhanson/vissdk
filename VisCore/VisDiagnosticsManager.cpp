...
{
	VisPGlobals()->SetLogFileName(szLogFileName);
}

char* CVisDiagnosticsManager::LogFileName(void)
{
	if(VisPGlobals()->LogFileName() == NULL)
	{
...
CVisConsoleWnd* CVisDiagnosticsManager::ConsoleWindow(void)
{
	...
	return VisPGlobals()->ConsoleWindow();
}
											...
											void* pvUser)
{
	...
	CVisDiagnosticsCallbackVector* vectorCallback = VisPGlobals()->DiagCallbackList();
...
{
	if(VisPGlobals()->DiagnosticsCallbackAvailable())
	{
		CVisDiagnosticsCallbackVector* vectorCallback = VisPGlobals()->DiagCallbackList();
...
void CVisDiagnosticsManager::TriggerUserFunction(char* szDescription, const char* szFileLine, 
												 ...
												 int nCur, int nLim)
{
	CVisGlobals* pvisGlobals = VisPGlobals();
			...
			(refcallbackCur.m_nDiagnosticsType & evisdiagnosticsType))
		{
			...
			switch(refcallbackCur.m_enumcallbackType)
			{
				...
				case evisdiagcallbackProgress:
				{
					...
					diagnosticsProgress(refcallbackCur.m_pvUser, dwLevel, szDescription, nCur, nLim);
					break;
				}
...
void CVisDiagnosticsManager::WriteToLogFile(const char* szDescription)
{
	...
	fwrite(szDescription, strlen(szDescription), 1, pLogFile);
	// Add a character-return and linefeed at the end of the line.
	fwrite("\xd\xa", 2, 1, pLogFile);
	fflush(pLogFile);
}