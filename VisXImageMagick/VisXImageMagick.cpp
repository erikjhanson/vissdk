...
#else // TAKEOUT
// This is the code that we currently use:
...
VisXImageMagickExport int __stdcall VisFWriteImageMagickHeader(
		SVisImageMagickData& refimagemagickdata)
{
	// UNDONE:  This function doesn't write grayscale images correctly.
#ifdef _DEBUG
    if (refimagemagickdata.m_cbitPixel == 8)
	{
		static bool s_fWarnedUser = false;
		if (!s_fWarnedUser)
		{
			OutputDebugString(
"VisXImageMagick Warning:  Grayscale images may not be written correctly.\n");
		}
	}
#endif // _DEBUG
		...
		SVisImageMagickData& refimagemagickdata)
{
	...
	// UNDONE:  This colormap code hasn't been tested with the new ImageMagick.
	// (Is it still needed?)
...
//				|| ((stricmp(image_info->magick, "gif") == 0))))
	{
		...
		else
		{
			...
			for (int iColor = 0; iColor < image->colors; ++iColor)
			{
				...
				image->colormap[iColor].opacity = 0;
			}
		}
