...
{
	int nProviders = VisPGlobals()->NImSrcProviders();
	if((*ppIVisListSz == 0) && (nProviders > 0))
		*ppIVisListSz = new CVisListSz;
	for(int iProvider = 0; iProvider < nProviders; iProvider++ )
	{
		std::string szProvider(VisPGlobals()->ImSrcProviderID(iProvider));
		szProvider += ": ";
		szProvider += VisPGlobals()->ImSrcProviderName(iProvider);
...
{
	std::string strBuf;
	const char *szProviderID = szProvider;
	const char *pchColon = strchr(szProvider, ':');
	if (pchColon != 0)
	{
		int ichColon = pchColon - szProvider;
		strBuf = szProvider;
		strBuf.erase(strBuf.begin() + ichColon, strBuf.end());
		szProviderID = strBuf.c_str();
	}
	...
	int iGlobal = VisPGlobals()->IGetProvider(szProviderID);
	if(iGlobal != -1)
	{
		VisPGlobals()->ImSrcProviderCallback(iGlobal)(0, szProviderID,
			...
			&hkeyDevs) == ERROR_SUCCESS)
	{
		...
		DWORD dwType;
		HKEY hkeySubkey;
		if (RegOpenKeyEx(hkeyDevs, szProviderID, 0, KEY_QUERY_VALUE,
					&hkeySubkey) == ERROR_SUCCESS)
		{
					...
					&& (dwType == REG_SZ))
			{
				...
				if ((pfnGetImSrcProvider != 0)
						&& (FAILED(pfnGetImSrcProvider(0, szProviderID,
								IID_IVisImSrcProvider,
								(void **) &pIVisImSrcProvider))))
				{
...
VisImSrcExport IVisImSrcProvider * VISAPI VisPIVisImSrcProviderGetI(int iProvider)
{
	...
	{
		const char *szSelect = pIVisListSzProvider->SzGetI(iProvider);
		pIVisImSrcProvider = VisPIVisImSrcProviderGetSz(szSelect);
