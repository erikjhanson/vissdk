		...
		SVisImSrcFrame *pimsrcframe)
{
	...
	if (pimsrcframe->m_pvData != 0)
	{
		if (fProcessImage)
		{
			...
			// Use a try-catch block to catch potential access violations that
			// we could get if a driver frees its image buffer before we copy the
			// data.
			// LATER:  We should probably add an option to copy the buffer in the
			// callback thread.
			CVisImageBase *pimage = 0;
			try
			{
				pimage = imagefromframe.PImageGetImSrcFrame(*pimsrcframe,
